...
class ServiceMiniRow(QFrame):
    ...
    def __init__(self, service: Dict, parent=None):
        ...
        if port:
            ...
            port_lbl.setStyleSheet("font-family: monospace; font-size: 11px; font-weight: 700; color: #2563EB; background: #EFF6FF; border: 1px solid #BFDBFE; border-radius: 6px; padding: 2px 6px;")
...
class StackCard(QFrame):
    ...
    def _init_ui(self):
        ...
        sub_lbl = QLabel(f"MULTI-PROJECT STACK ({len(self.stack.get('services', []))} SERVICES)")
        sub_lbl.setStyleSheet("font-size: 10px; font-weight: 800; color: #6B7280; letter-spacing: 0.5px; background: transparent;")
    ...
    def set_service_status(self, service_id: str, status: str):
        if service_id in self.service_rows:
            self.service_rows[service_id].set_status(status)
